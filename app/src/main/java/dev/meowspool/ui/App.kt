...
private fun Sheet(current: Dest, go: (Dest) -> Unit) {
    ...
    @Composable fun item(d: Dest) = NavigationDrawerItem(
        ...
        shape = CircleShape, // MD3 spec: nav drawer item active indicator uses the "full" (pill) shape.
        colors = NavigationDrawerItemDefaults.colors(),
    )
    // MD3 spec: modal navigation drawer uses the "large" (16dp) end-corner shape token.
    val drawerShape = MaterialTheme.shapes.large.copy(topStart = CornerSize(0.dp), bottomStart = CornerSize(0.dp))
    ModalDrawerSheet(drawerShape = drawerShape) {
        Column(Modifier.verticalScroll(rememberScrollState()).padding(bottom = 12.dp)) {
            Row(Modifier.padding(horizontal = 28.dp, vertical = 24.dp), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(14.dp)) {
                Box(Modifier.size(44.dp).background(cs.primaryContainer, MaterialTheme.shapes.medium), contentAlignment = Alignment.Center) {
